...
const uint8_t UBX_SETMAXPERFORMANCE[] = {
	0xB5, 0x62, 0x06, 0x11, 0x02, 0x00, 0x08, 0x00, 0x21, 0x91
};

//Power Save Mode
const uint8_t UBX_SETPOWERSAVEMODE[] = {
		0xB5, 0x62, 0x06, 0x11, 0x02, 0x00, 0x08, 0x01, 0x22, 0x92
};

const uint8_t UBX_SETCYCLICTRACKING[] = {
...
const uint8_t UBX_SETCYCLICTRACKINGLONG[] = {
	0xB5, 0x62, 0x06, 0x3B, 0x2C, 0x00, 0x01, 0x06, 0x00, 0x00,
...
const uint8_t NEW_AIRBORNE_MODEL[] = {
...
const uint8_t NEW_CONFIGURE_POWERSAVE[] = {
...
{
	uint8_t i;
	for(i=0; i<sizeof(NEW_SET_NMEA); i++) {
		UART_TransmitChar(NEW_SET_NMEA[i]);
		delay(1);
	}
}
...
{
	uint8_t i;
	for(i=0; i<sizeof(NEW_GPS_ONLY); i++) {
		UART_TransmitChar(NEW_GPS_ONLY[i]);
		delay(1);
	}
}
...
{
	uint8_t i;
	for(i=0; i<sizeof(NEW_AIRBORNE_MODEL); i++) {
		UART_TransmitChar(NEW_AIRBORNE_MODEL[i]);
		delay(1);
	}
}
...
{
	uint8_t i;
	for(i=0; i<sizeof(NEW_CONFIGURE_POWERSAVE); i++) {
		UART_TransmitChar(NEW_CONFIGURE_POWERSAVE[i]);
		delay(1);
	}
}
...
{
	uint8_t i;
	for(i=0; i<sizeof(NEW_SWITCH_POWERSAVE_ON); i++) {
		UART_TransmitChar(NEW_SWITCH_POWERSAVE_ON[i]);
		delay(1);
	}
}
...
{
	uint8_t i;
	for(i=0; i<sizeof(NEW_SWITCH_POWERSAVE_OFF); i++) {
		UART_TransmitChar(NEW_SWITCH_POWERSAVE_OFF[i]);
		delay(1);
	}
}
